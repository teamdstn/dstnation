var DomHandler = require("./DomHandler.js"), DomUtils = require("./DomUtils.js");
...
function getElements(what, where, one, recurse) {
    if (one) return DomUtils.getElementsByTagName(what, where, recurse, 1)[0]; else return DomUtils.getElementsByTagName(what, where, recurse);
}

function fetch(what, where, recurse) {
    var ret = getElements(what, where, true, recurse);
    if (ret && (ret = ret.children) && ret.length !== 0) return ret[0].data; else return false;
}
...
FeedHandler.prototype.onend = function() {
    var feed = {}, feedRoot = getElements(isValidFeed, this.dom, true), tmp, childs;
    if (feedRoot) {
