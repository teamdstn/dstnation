...
module.exports = exports = function fieldsAliasPlugin(schema, options) {
	...
	for(var p in schema.paths){
		...
		if( schema.paths[p].options.alias && 'string' == typeof schema.paths[p].options.alias && schema.paths[p].options.alias != ''){
			var aliased_property = schema.paths[p].options.alias;
			var real_property = schema.paths[p].path;
