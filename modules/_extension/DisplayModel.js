...
function find(field, args) {
  for (var i = 0, l = args.length; i < l; i++) {
    var arg = args[i];
    if (!(_u.isUndefined(arg) || _u.isUndefined(arg[field]))) return arg[field];
  }
}
...
function findAll(field, args) {
  ...
  for (var i = 0, l = args.length; i < l; i++) {
    var arg = args[i];
    if (!(_u.isUndefined(arg) || _u.isUndefined(arg[field]))) ret.push(arg[field]);
  }
...
var global_options = {
  ...
  builtin_editors: ["Checkbox", "Checkboxes", "Date", "DateTime", "Hidden", "List", "NestedModel", "Number", "Object", "Password", "Radio", "Select", "Text", "TextArea"]
};
...
App.prototype.modelFor = function (model) {
  var Model = model;
  if (typeof model == "string") Model = this.modelPaths[model];
  else if (model && model.modelName) Model = this.modelPaths[model.modelName];
  return Model;
};
...
function Model(modelName, args) {
  ...
  this.__defineGetter__("labelAttr", function onLabelAttrGet() {
    ...
    for (var i = 0, l = labels.length; i < l; i++) {
      if (this.fields.indexOf(labels[i]) > -1) return labels[i];
    }
  });
  ...
  this.__defineGetter__("list_fields", function onListFieldsGet() {
    ...
    if (!fields) fields = this.fields.filter(function (v, k) {
...
Model.prototype.finder = function onFindFinder(name) {
  return _u(this.finders).find(function (f) {
    return f.name == name;
  }) || {};
};
...
Model.prototype.fieldsFor = function (list_type) {
  if (!list_type) list_type = this.fields;
...
Model.prototype.editorsFor = function onEditorsFor(fields, schema) {
  ...
  _u(fields || this.edit_fields).each(function (v, k) {
    if (v.fields) flat = flat.concat(v.fields);
    else flat.push(v);
  });
  _u(flat).each(function (k, v) {
    ...
    if (c) do {
      var type = c.type;
      if (type && !~global_options.builtin_editors.indexOf(type)) editors[inflection.hyphenize(type)] = true;
      c = c.subSchema;
    } while (c && c.subSchema);
  });
