var encode = function() {
  ...
  ;
    ...
    function encode(rootNode, data, delimiter, nodeCallback, useIdIfEmptyName) {
        ...
        for (var i = 0; i < fieldValues.length; i++) {
            var fieldName = fieldValues[i].name, fieldValue = fieldValues[i].value;
    ...
    function getFields(rootNode, useIdIfEmptyName, delimiter, arrayIndexes, shouldClean) {
        ...
        var result = {}, currNode = rootNode.firstChild, name, nameNormalized, subFieldName, i, j, l, options;
        while (currNode) {
            name = "";
    ...
    function normalizeName(name, delimiter, arrayIndexes) {
        var nameChunksNormalized = [], nameChunks = name.split(delimiter), currChunk, nameMatches, nameNormalized, currIndex, newIndex, i;
