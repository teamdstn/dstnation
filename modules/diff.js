  ...
  var diff = {
      ...
      stringAsLines: function(str) {
          ...
          for (var i = 0; i < lines.length; i++) {
              lines[i] = diff.stripLinebreaks(lines[i]);
          }
          return lines;
      },
      ...
      SequenceMatcher: function(a, b, isjunk) {
          ...
          this.__chain_b = function() {
              ...
              this.isbjunk = diff.__isindict(junkdict);
              this.isbpopular = diff.__isindict(populardict);
          };
          this.find_longest_match = function(alo, ahi, blo, bhi) {
              ...
              for (var i = alo; i < ahi; i++) {
                  var newj2len = {};
                  var jdict = diff.__dictget(b2j, a[i], nothing);
                  for (var jkey in jdict) {
                      if (jdict.hasOwnProperty(jkey)) {
                          j = jdict[jkey];
                          if (j < blo) continue;
                          if (j >= bhi) break;
                          newj2len[j] = k = diff.__dictget(j2len, j - 1, 0) + 1;
                          if (k > bestsize) {
                              besti = i - k + 1;
                              bestj = j - k + 1;
                              bestsize = k;
          ...
          this.get_matching_blocks = function() {
              ...
              matching_blocks.sort(diff.__ntuplecomp);
          ...
          this.ratio = function() {
              matches = diff.__reduce(function(sum, triple) {
                  return sum + triple[triple.length - 1];
              }, this.get_matching_blocks(), 0);
              return diff.__calculate_ratio(matches, this.a.length + this.b.length);
          };
          this.quick_ratio = function() {
              ...
              if (this.fullbcount == null) {
                  ...
                  for (var i = 0; i < this.b.length; i++) {
                      elt = this.b[i];
                      fullbcount[elt] = diff.__dictget(fullbcount, elt, 0) + 1;
                  }
              }
              fullbcount = this.fullbcount;
              var avail = {};
              var availhas = diff.__isindict(avail);
              var matches = numb = 0;
              for (var i = 0; i < this.a.length; i++) {
                  ...
                  if (availhas(elt)) {
                      ...
                      numb = diff.__dictget(fullbcount, elt, 0);
                  }
                  avail[elt] = numb - 1;
                  if (numb > 0) matches++;
              }
          ...
          this.isjunk = isjunk ? isjunk : diff.defaultJunkFunction;
          this.a = this.b = null;
          this.set_seqs(a, b);
      }
  };
