...
function File(options) {
    ...
    fs.stat(options.path, function(err, stats) {
        var finish = function(err, stats) {
            ...
            if (self.src && self.src.method && !self.writable && self.dest.writeHead && (self.src.method === "PUT" || self.src.method === "POST")) {
                self.dest.writeHead(405, {
            ...
            if (err && self.dest && self.dest.writeHead && (self.src && (self.src.method == "GET" || self.src.method === "HEAD") || !self.src)) {
                if (err.code === "ENOENT") {
            ...
            if (self.src && (self.src.method === "GET" || self.src.method === "HEAD") && self.dest) {
                ...
                if (self.dest.writeHead) {
                    if (self.src && self.src.headers) {
                        if (self.src.headers["if-none-match"] === self.etag || self.src.headers["if-modified-since"] === self.lastmodified) {
