...
var Classfy = function(options) {
  ...
  switch(backend.type.toLowerCase()) {
    case 'redis':
      this.backend = new (require("./_classfy/redis").RedisBackend)(backend.options);
      break;
    case 'localstorage':
      this.backend = new (require("./_classfy/localStorage")
                     .LocalStorageBackend)(backend.options);
      break;
    default:
      this.backend = new (require("./_classfy/memory").MemoryBackend)();
  }
}
