...
var _xrds = function(str) {
  return convert.xrds.parse(str);
};
...
openid.saveAssociation = function (provider, type, handle, secret, expiry_time_in_seconds, callback) {
  ...
  }, expiry_time_in_seconds * 1e3);
  _associations[handle] = {
    provider: provider,
    type: type,
    secret: secret
  };
...
var _get = function (getUrl, params, callback, redirects) {
    ...
    var options = {
      ...
      port: _isDef(getUrl.port) ? parseInt(getUrl.port, 10) : getUrl.protocol == "https:" ? 443 : 80,
      path: path
    };
...
var _post = function (postUrl, data, callback, redirects) {
    ...
    var options = {
      ...
      path: path,
      port: _isDef(postUrl.port) ? postUrl.port : postUrl.protocol == "https:" ? 443 : 80,
      headers: {
...
openid.associate = function (provider, callback, strict, algorithm) {
  ...
  _post(provider.endpoint, params, function (data, headers, statusCode) {
    ...
    if (data.error_code == "unsupported-type" || !_isDef(data.ns)) {
      if (algorithm == "DH-SHA1") {
        if (strict && url.protocol != "https:") {
          return callback({
            message: "Channel is insecure and no encryption method is supported by provider"
          }, null);
        } else {
    ...
    if (data.error) {
      ...
      openid.saveAssociation(provider, hashAlgorithm, data.assoc_handle, secret, data.expires_in * 1, function (error) {
        if (error) {
          return callback(error);
        }
        callback(null, data);
...
openid.authenticate = function (identifier, returnUrl, realm, immediate, stateless, callback, extensions, strict) {
  openid.discover(identifier, strict, function (error, providers) {
    ...
    var chooseProvider = function successOrNext(error, authUrl) {
        if (!error && authUrl) {
          ...
          if (provider.claimedIdentifier) {
            ...
            if (useLocalIdentifierAsKey && !provider.localIdentifier) {
              return callback({
                message: "Cannot retain discovered information; the provider does not contain the required attributes"
              });
            }
            ...
            successOrNext({
              message: "OpenID 1.0/1.1 provider cannot be used without a claimed identifier"
            });
          }
        }
        ...
        if (stateless) {
          _requestAuthentication(currentProvider, null, returnUrl, realm, immediate, extensions || {}, successOrNext);
        } else {
          openid.associate(currentProvider, function (error, answer) {
            if (error || !answer || answer.error) {
              successOrNext(error || answer.error, null);
            } else {
              _requestAuthentication(currentProvider, answer.assoc_handle, returnUrl, realm, immediate, extensions || {}, successOrNext);
            }
          });
        }
      };
...
var _requestAuthentication = function (provider, assoc_handle, returnUrl, realm, immediate, extensions, callback) {
    ...
    if (realm) {
      ...
      return callback({
        message: "No return URL or realm specified"
      });
    }
...
var _verifyAssertionData = function (params, callback, stateless, extensions, strict) {
    ...
    if (!_invalidateAssociationHandleIfRequested(params)) {
      return callback({
        message: "Unable to invalidate association handle"
      });
    }
...
var _verifyDiscoveredInformation = function (params, stateless, extensions, strict, callback) {
    ...
    openid.loadDiscoveredInformation(claimedIdentifier, function (error, provider) {
      if (error) {
        return callback({
          message: "An error occured when loading previously discovered information about the claimed identifier"
        });
      }
      if (provider) {
        ...
        return callback({
          message: "OpenID 1.0/1.1 response received, but no information has been discovered about the provider. It is likely that this is a fraudulent authentication response."
        });
      }
      openid.discover(claimedIdentifier, strict, function (error, providers) {
        ...
        if (!providers || !providers.length) {
          return callback({
            message: "No OpenID provider was discovered for the asserted claimed identifier"
          });
        }
        ...
        return callback({
          message: "No providers were discovered for the claimed identifier"
        });
      });
    });
  };
...
var _verifyAssertionAgainstProvider = function (provider, params, stateless, extensions, callback) {
    if (provider.version.indexOf("2.0") !== -1) {
      ...
      if (provider.endpoint != endpoint) {
        return callback({
          message: "OpenID provider endpoint in assertion response does not match discovered OpenID provider endpoint"
        });
      }
      if (provider.claimedIdentifier) {
        ...
        if (provider.claimedIdentifier != claimedIdentifier) {
          return callback({
            message: "Claimed identifier in assertion response does not match discovered claimed identifier"
          });
        }
      }
    }
    if (provider.localIdentifier && provider.localIdentifier != params["openid.identity"]) {
      return callback({
        message: "Identity in assertion response does not match discovered local identifier"
      });
    }
...
var _checkSignatureUsingAssociation = function (params, callback) {
    openid.loadAssociation(params["openid.assoc_handle"], function (error, association) {
      if (error) {
        return callback({
          message: "Error loading association"
        }, {
          authenticated: false
        });
      }
      if (!association) {
        return callback({
          message: "Invalid association handle"
        }, {
          authenticated: false
        });
      }
      if (association.provider.version.indexOf("2.0") !== -1 && association.provider.endpoint !== params["openid.op_endpoint"]) {
        return callback({
          message: "Association handle does not match provided endpoint"
        }, {
          authenticated: false
        });
      }
      ...
      for (var i = 0; i < signedParams.length; i++) {
        ...
        if (!_isDef(value)) {
          return callback({
            message: "At least one parameter referred in signature is not present in response"
          }, {
            authenticated: false
          });
        }
      ...
      if (ourSignature == params["openid.sig"]) {
        callback(null, {
          authenticated: true,
          claimedIdentifier: association.provider.version.indexOf("2.0") !== -1 ? params["openid.claimed_id"] : association.provider.claimedIdentifier
        });
      } else {
        callback({
