...
var CStream     = mtrude.CStream;
var BufferChain = mtrude.utils.BufferChain;
var EventEmitter = events.EventEmitter;
function dbg() {
  if (MStream.DBG) console.log.apply(console, arguments);
}
/**
 * RTMP MStream: a low-level data-agnostic layer of RTMP. MStream
 * delivers a stream of messages with the event 'message'. Also a message can
 * be sent. In both cases chunking and de-chunking is done by MStream
 * and CStream in tandem.
 *
 * MStream emits these events:
 ...
 * MStream has these methods:
 *   send(message) send a message
 *   close()       close the connection
 *
 * MStream has these properties
 *   chunkStream the underlying chunk stream. Readonly.
 *
*/
function MStream(chunkStream) {
  var it = this;
  ...
  it.chunkStream.on('warn', function(msg) { it.warn('CStream: ' + msg); });
};
util.inherits(MStream, EventEmitter);
MStream.DBG = false;
// === Prototype ==
var p = MStream.prototype;
// message: csid, msid, typeid, timestamp, data
p.send = function(typeid, msid, csid, timestamp, data) {
  if (data.length < this.chunkStream.chunkSize) {
    var chunk = {
      typeid: typeid, msid: msid, csid: csid, timestamp: timestamp, data: data
    };
    this.chunkStream.send(chunk);
  }
  else {
    throw new Error('Splitting in chunks not supported');
  }
}
p.handleChunk = function(chunk) {
  ...
  if (csid == 2){
    this.warn('Non-control message '+ (rtmp.typeNames[typeid] || '#' + typeid) + ' with csid=2.' + continuing);
  }
...
p.handlePing = function(chunk) {
  ...
  dbg('PING :', 'type :', ping.type, 'msid', ping.msid);
  switch (type) {
    case pings.BEGIN:
    case pings.EOF:
    case pings.DRY:
...
p.controlProtocol = function(chunk) {
  ...
  switch (chunk.typeid) {
    case types.CHUNK_SIZE:
      ...
      break;
    case types.ABORT:
      this.warn('ABORT Protocol Control received. Closing CStream.');
...
Object.freeze(MStream.prototype);
Object.seal(MStream);
