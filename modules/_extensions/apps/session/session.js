...
SessionPersistence.prototype.save = function (key, data, callback) {
  if (this.session) {
    var conf = (this.session.conf || (this.session.conf = {
      plugins: {}
    })).plugins[key] = data;
...
SessionPersistence.prototype.list = function (callback) {
  if (this.orig) return this.orig.list(callback);
}
...
SessionPersistence.prototype.read = function (filename) {
  if (this.session) {
    ...
    if (this.orig) return this.orig.read(filename);
  }
}
...
util.inherits(SessionPlugin, Proto);
