(function(root, undefined) {
  ...
  var __d = (function(Date, Math, Array, undefined) {
    ...
    var proto = __d.prototype;
    // This makes an __d look pretty in Firebug and Web Inspector.
    // It makes an __d seem array-like, and displays [ <internal-date>.toString() ]
    proto.length = 1;
    ...
    function __d() {
      return init(
        (this instanceof __d) ? this : new __d(),
        arguments
      );
    }
    ...
    function _diff(xdate1, xdate2, fieldIndex) { // fieldIndex=FULLYEAR is for years, fieldIndex=DATE is for days
      ...
      xdate2 = __d(xdate2).setUTCMode(true, true);
      var v = 0;
    ...
    __d.parse = function(str) {
      return +__d(''+str);
    };
    function parse(str, utcMode, xdate) {
      var parsers = __d.parsers;
      var i = 0;
      var res;
    ...
    function format(xdate, formatString, settings, uniqueness, useUTC) {
      var locales = __d.locales;
      var defaultLocaleSettings = locales[__d.defaultLocale] || {};
      var getField = curry(_getField, xdate, useUTC);
    ...
    function getTokenReplacement(xdate, token, getField, getSetting, useUTC) {
      var formatter = __d.formatters[token];
    ...
    proto.clearTime = function() {
      return this.setHours(0, 0, 0, 0); // will return an __d for chaining
    };
    ...
    __d.today = function() {
      return new __d().clearTime();
    };
    __d.UTC = UTC;
    __d.getDaysInMonth = getDaysInMonth;
    /* Internal Utilities
    ---------------------------------------------------------------------------------*/
  ...
  var __instance = __d, __name = "date", __nms = "$g";
